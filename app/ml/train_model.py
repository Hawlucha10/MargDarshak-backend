"""
MargDarshak ML: Production Tri-Model Ensemble Training Pipeline
================================================================
- 57 features: 41 raw + 8 domain interactions + 6 physical railway + 2 advanced encodings
- Tri-Model Stacking: XGBoost (GPU) + LightGBM (GPU) + CatBoost
...
- Dual Head: Classification (is_delayed, AUC-ROC) + Regression (delay_minutes, MAE)
...
# =========================================================================
# STEP 2: Feature Engineering (14 new features)
# =========================================================================
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    log_section("STEP 2: Feature Engineering (14 Interaction Features)")
    t0 = time.time()
    ...
    log(f"Engineered 14 features in {time.time()-t0:.2f}s")
    return df
...
def prepare_features(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series, pd.Series]:
    ...
    # Drop targets, IDs, leakage columns
    drop_cols = [c for c in COLUMNS_TO_DROP if c in df.columns]
    # Also drop departure_date (string date, not useful as numeric feature)
    if "departure_date" in df.columns:
        drop_cols.append("departure_date")
    # Drop train_number (already encoded via target encoding)
    if "train_number" in df.columns:
        drop_cols.append("train_number")

    X = df.drop(columns=drop_cols)

    # Convert any remaining object cols to numeric (safety net)
...
def train_ensemble_classification(
    ...
    """
    Trains XGBoost, LightGBM, CatBoost with 5-fold CV.
    Returns trained models and OOF predictions for meta-learner.
    """
    ...
    n_samples = len(X)
    oof_preds = np.zeros((n_samples, 3))  # 3 models
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
        ...
        log(f"  XGBoost  AUC={xgb_auc:.5f}  ({time.time()-t0:.1f}s, "
        ...
        log(f"  LightGBM AUC={lgb_auc:.5f}  ({time.time()-t0:.1f}s, "
        ...
        log(f"  CatBoost AUC={cat_auc:.5f}  ({time.time()-t0:.1f}s, "
...
# =========================================================================
# STEP 9: Train Regression Head (delay_minutes)
# =========================================================================
def train_regression_head(X: pd.DataFrame, y: pd.Series) -> dict:
    import xgboost as xgb

    log_section("STEP 9: Regression Head (delay_minutes)")
    t0 = time.time()

    kf = StratifiedKFold(n_splits=N_FOLDS, shuffle=True, random_state=RANDOM_STATE)
    # Bin delay_minutes for stratified split
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X, y_binned)):
        ...
        pred = np.clip(pred, 0, None)  # No negative delays
...
def save_artifacts(
    ...
    log_section("STEP 10: Saving Model Artifacts")

    # Save each fold's models
...
def print_feature_importance(all_models: dict, feature_names: list) -> None:
    log_section("STEP 11: Top 20 Feature Importances (XGBoost avg across folds)")
...
def main():
    ...
    log_section("MargDarshak ML: Production Training Pipeline")
    log("Target: is_delayed (AUC-ROC) + delay_minutes (MAE)")
    log("Ensemble: XGBoost (GPU) + LightGBM (GPU) + CatBoost (GPU)")
    ...
    # 9. Train regression head (delay_minutes)
    reg_result = train_regression_head(X, y_reg)

    # 10. Save all artifacts
    save_artifacts(all_models, meta_learner, reg_result, label_encoders, feature_names)
    ...
    log("Ready for production inference!")
